...
#%% Configuration
window_time = 0.5          # training time use for the fft in seconds
...
for i in range(0,i_limit):#i_limit):
    ...
    frequency_peaks_height = 0.01     # percent of the peak measured from the max peak
    frequency_peaks_distance = 1   # bins away that a new peak must be
    ...
    if zero_pad == True:
        N_xx = xx.size
        zeroN = next_power_of_2(N_xx) - N_xx
        totalN = zeroN + N_xx
        zeropadded_xx = np.zeros(zeroN + N)
        zeropadded_xx[:N_xx] = xx
        

        # Number of sample points
        N = zeropadded_xx.size
        yf = 2.0/N * np.abs(sp.fft.fft(zeropadded_xx)[0:N//2])
        xf = sp.fft.fftfreq(N, dt)[:N//2]
    
    else:
        # Number of sample points
        N = xx.size
        yf = 2.0/N * np.abs(sp.fft.fft(xx)[0:N//2])
        xf = sp.fft.fftfreq(N, dt)[:N//2]
    ...
    #frequency_peaks_bins = sp.signal.find_peaks(yf)
    
    frequency_list = xf[frequency_peaks_bins[0]]
    frequency_list_conjugated = np.hstack((np.asarray(frequency_list),np.asarray(frequency_list)*-1))
    frequencys_used.append(frequency_list)
    
    # Returns the vector of data up to forcast
    signal_pred.append(partMLC.fft_prediction(xx,dt,forecast_horizon_steps,
                                              frequency_list_conjugated,
                                              returnVector=False))
...
for i in range(len(frequencys_used)):
    plt.plot(tt_signal_pred[i]*np.ones(len(frequencys_used[i])),frequencys_used[i],'ko',markersize=0.1)
